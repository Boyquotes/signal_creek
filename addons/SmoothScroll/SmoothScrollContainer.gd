...
export var over_drag_multiplicator_top := 1
# Current counterforce for "overdragging" on the bottom
export var over_drag_multiplicator_bottom := 1
...
func smooth_scroll_to_bottom() -> void:
		velocity.y -= speed
...
func scroll_to_pos(amount: float):
	# Reset velocity
	velocity.y = 0
	# Move content node to bottom
	pos.y = -content_node.rect_size.y - amount + self.rect_size.y
	content_node.rect_position = pos
	# Update vertical scroll bar
	set_v_scroll(-pos.y)
